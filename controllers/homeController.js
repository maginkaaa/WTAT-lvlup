...
exports.renderPage = async (req, res) => {
    ...
    let summoner = await getSummoner(summonerName, region);
...
exports.getLiveMatch = async (req, res) => {
    ...
    if (summoner == null) {
        res.status(404);
        res.send({status : "Player not found"});
        return;
    }
    ...
    if (match != null) {
        res.status(200);
        let blueTeamIds = [];
        for (let i = 0; i < match.blueTeam.participants.length; i++) {
            blueTeamIds.push(match.blueTeam.participants[i].name);
        }
        let redTeamIds = [];
        for (let i = 0; i < match.redTeam.participants.length; i++) {
            redTeamIds.push(match.redTeam.participants[i].name);
        }
        res.send({status : "Player in match", match: {id: match.gameId, gameMode: match.gameMode, blueTeam: blueTeamIds, redTeam: redTeamIds}});
    }
    else {
        res.status(404);
        res.send({status : "Not in a match"});
    }
};